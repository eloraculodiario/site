...
// Para 40 min usamos pasos de 20 (mcm 60-40=20) para grilla fluida.
const configFranjas = {
...
// Mapeo de servicios (alineado con index.html)
const serviciosInfo = {
...
function calcularHorariosDisponibles(fecha, config) {
  ...
  // helper: descarta slots con menos de 2h de antelación si es hoy
  ...
  for (let hora = config.inicioTarde; hora < config.finTarde; hora++) {
    for (const minuto of config.intervalos) {
      // Evitar desbordar más allá de finTarde por la duración del servicio
...
function estaHorarioOcupado(fecha, horario, franja) {
  ...
  const [hIni, mIni] = horario.split(':').map(Number);
  const inicioReserva = new Date(0, 0, 0, hIni, mIni);
  const duracionMin = parseInt(franja, 10) || 30;
  const finReserva = new Date(inicioReserva.getTime() + duracionMin * 60000);

  // Consideramos reservas existentes de 30 min (simulación)
  return horariosOcupados[fecha].some(ocupado => {
    const [ho, mo] = ocupado.split(':').map(Number);
    const ocupadoInicio = new Date(0, 0, 0, ho, mo);
    const ocupadoFin = new Date(ocupadoInicio.getTime() + 30 * 60000);
...
function validarFormulario() {
  ...
  // Validación extra: antelación mínima en el submit (por si el usuario mantuvo la página abierta)
...
function cargarReservaTemporal() {
  try {
    ...
    console.error('❌ Error cargando reserva temporal:', e);
